...
from ENTITY_RESOLVER import match_entities, extract_internship_rows
...
def test_match_entities_handles_missing_institution_names():
    import pandas as pd

    df = pd.DataFrame([
        {"institution_name": "IIT Delhi", "state": "Delhi"},
        {"institution_name": None, "state": "Tamil Nadu"},
        {"institution_name": "", "state": ""},
    ])

    resolved = match_entities(df)

    assert "master_entity_id" in resolved.columns
    assert "match_score" in resolved.columns
    assert resolved["master_entity_id"].notna().all()
    assert resolved["match_score"].notna().all()


def test_extract_internship_rows_handles_missing_internship_fields():
    import pandas as pd

    df = pd.DataFrame([
        {"institution_name": "IIT Delhi", "state": "Delhi"},
        {"institution_name": "IIT Bombay", "state": "Maharashtra"},
    ])

    internships = extract_internship_rows(df)

    assert isinstance(internships, pd.DataFrame)
    assert list(internships.columns) == [
